...
const monitorIncomingTexts = async () => {
    ...
    while (true) {
        ...
        if (newMessages.length > 0) {
            ...
            const sender = await latestMessage.locator('div[data-e2e="message-sender"]').textContent();
            const messageText = await latestMessage.locator('div[data-e2e="message-content"]').textContent();

            console.log(`New message from ${sender}: ${messageText}`);

            await axios.post(WEBHOOK_URL, { sender, message: messageText }).catch(err => {
        ...
        if (req.method === "POST" && req.url.endsWith("/send")) {
            try {
                ...
                if (!recipient || !message) {
                    return new Response(JSON.stringify({ error: "Missing recipient or message" }), { status: 400 });
                }

                await sendMessage(recipient, message);
                return new Response(JSON.stringify({ status: "Message sent" }), { status: 200 });
            } catch (error) {
